...
def main():
    ...
    parser.add_argument("--takes", type=positive_int, default=5,
    ...
    parser.add_argument("--preset", type=int, choices=range(1, 11), default=None,
    ...
    parser.add_argument("--split", choices=("pilot", "dev", "test"), default="pilot",
    ...
    parser.add_argument("--condition", default="quiet_normal_voice",
                        help="Điều kiện thu âm (mặc định 'quiet_normal_voice', 'quiet_far', 'normal_noise').")
    parser.add_argument("--manual-advance", action="store_true",
    ...
    # Session ID
    now = datetime.now()
    session_id = args.session_id or f"S_{now.strftime('%Y%m%d_%H%M%S')}_{args.speaker}"
    ...
    def sync_to_child_study():
        ...
        try:
            ...
            # Register each clip in labels.jsonl
            for idx, clip in enumerate(manifest["clips"], start=1):
                ...
                save_label(label_entry)
        except Exception as exc:
            print(f"[WARN] Không thể đồng bộ nhãn: {exc}", file=sys.stderr)
    ...
    try:
            ...
            with AlsaCapture(config.device) as capture:
                ...
                for take in range(1, args.takes + 1):
                    ...
                    if take < args.takes and not args.manual_advance:
                        # 1.5s interval between automatic takes
                        for _ in range(75):
                            capture.read_frame()
        ...
        print(f"\n[INTERRUPT] Đã ngắt thu giữa chừng. Đã lưu {len(manifest['clips'])} take(s).", flush=True)
    except Exception as exc:
        manifest["status"] = "interrupted"
