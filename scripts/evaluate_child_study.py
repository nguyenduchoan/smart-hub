...
Mặc định chỉ dùng mẫu đã được duyệt (accepted + confirmed) trên tập dev.
Integrity của WAV được chấm trong evaluator để lỗi vẫn nằm trong denominator.
...
from smart_hub.child_study_policy import (
    select_samples_for_evaluation,
    finalize_evaluation_results,
    format_report_without_aggregate_targets,
)
...
def _load_ad_hoc_samples(args, config):
    raw_samples = []
    if args.wav:
...
def main():
    ...
    if args.wav or args.dir:
        if not args.allow_unreviewed:
            print(
                ...
                "Cần truyền thêm cờ --allow-unreviewed để chạy.", file=sys.stderr,
            )
            sys.exit(2)
        evaluation_mode = "ad_hoc"
        raw_samples = _load_ad_hoc_samples(args, config)
    else:
    ...
    eligible_samples, ineligible = select_samples_for_evaluation(
        raw_samples,
        split=args.split,
    ...
    if not eligible_samples:
        print(
            ...
            f"(split={args.split}, session={args.session_id}, speaker={args.speaker}, "
            f"label={args.label}, mode={evaluation_mode}).", file=sys.stderr,
        )
        if ineligible:
            print(f"Tổng cộng {len(ineligible)} mẫu bị loại bởi metadata/review policy:", file=sys.stderr)
    ...
    print(
        ...
        f"(mode={evaluation_mode}, split={args.split})...", flush=True,
    )
    ...
    finalize_evaluation_results(results)
    md_report = format_report_without_aggregate_targets(format_evaluation_markdown(results))

    try:
        out_file = save_evaluation_results(results, args.output, force=args.force)
        out_file.with_suffix(".md").write_text(md_report, encoding="utf-8")
